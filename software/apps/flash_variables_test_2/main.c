...
static ret_code_t fds_write(uint16_t file_id, uint16_t record_key, void const *p_data) {
    fds_record_t record;
    fds_record_desc_t record_desc;
    record.file_id = file_id;
    record.key = record_key;
    record.data.p_data = p_data;
    record.data.length_words = (sizeof(*p_data) + 3) / 4; // 4 bytes = 1 word; we take the ceiling of the # of words
    return fds_record_write(&record_desc, &record);
}

static fds_flash_record_t fds_read(uint16_t file_id, uint16_t record_key) {
