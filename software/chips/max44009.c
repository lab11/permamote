...
static nrf_twi_mngr_transfer_t const threshold_write_transfer[] = {
  NRF_TWI_MNGR_WRITE(MAX44009_ADDR, thresh_buf, 2, NRF_TWI_MNGR_NO_STOP),
};
...
void calc_mant_exp (float lux, bool upper, uint8_t* exp, uint8_t* mant) {
  static uint16_t max_mantissa = 128+64+32+16;
  *mant = 0;
  if(upper) {
    *mant += 15;
    max_mantissa += 15;
  ...
  if(lux < 11.5) {
    ...
    *exp = (uint8_t) ceil(log2(remainder));
    if (*exp > 0xE) *exp = 0xE;
  }
...
void max44009_set_upper_threshold(float upper_threshold) {
  uint8_t exp, mant;
...
void max44009_set_lower_threshold(float lower_threshold) {
  uint8_t exp, mant;
