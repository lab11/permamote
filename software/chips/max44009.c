...
static void interrupt_handler(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action) {
  int error = nrf_twi_mngr_perform(twi_mngr_instance, int_status_transfer, sizeof(int_status_transfer)/sizeof(int_status_transfer[0]), NULL);
...
void max44009_enable_interrupt(void) {
  int_enable_buf[1] = 1;
  int error = nrf_twi_mngr_perform(twi_mngr_instance, int_time_write_transfer, sizeof(int_time_write_transfer)/sizeof(int_time_write_transfer[0]), NULL);
  APP_ERROR_CHECK(error);
  error = nrf_twi_mngr_perform(twi_mngr_instance, int_enable_transfer, sizeof(int_enable_transfer)/sizeof(int_enable_transfer[0]), NULL);
  APP_ERROR_CHECK(error);
...
void max44009_disable_interrupt(void) {
  int_enable_buf[1] = 0;
  int error = nrf_twi_mngr_perform(twi_mngr_instance, int_enable_transfer, sizeof(int_enable_transfer)/sizeof(int_enable_transfer[0]), NULL);
  APP_ERROR_CHECK(error);
...
void max44009_config(max44009_config_t config) {
  ...
  int error = nrf_twi_mngr_perform(twi_mngr_instance, config_write_transfer, sizeof(config_write_transfer)/sizeof(config_write_transfer[0]), NULL);
  APP_ERROR_CHECK(error);
}
...
void max44009_set_upper_threshold(float thresh) {
  ...
  int error = nrf_twi_mngr_perform(twi_mngr_instance, threshold_write_transfer, sizeof(threshold_write_transfer)/sizeof(threshold_write_transfer[0]), NULL);
  APP_ERROR_CHECK(error);
...
float max44009_read_lux(void) {
  nrf_twi_mngr_perform(twi_mngr_instance, lux_read_transfer, sizeof(lux_read_transfer)/sizeof(lux_read_transfer[0]), NULL);
  return calc_lux();
}
