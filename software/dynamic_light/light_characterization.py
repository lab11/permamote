...
def on_message(client, userdata, msg):
    ...
    if state == 'baseline':
        if device_id not in devices:
            devices[device_id] = LightSensor(device_id)
    ...
    if state == 'characterizing':
        ...
        if current_light == None:
            ...
            return
        elif current_light not in devices[device_id].light_char_measurements:
            devices[device_id].light_char_measurements[current_light] = []
        measurements = [x[0] for x in devices[device_id].light_char_measurements[current_light]]
        if current_brightness not in measurements:
            devices[device_id].light_char_measurements[current_light].append([current_brightness, lux])
...
for device in devices:
    ...
    for light_source in devices[device].light_char_measurements:
        measurements = np.array(devices[device].light_char_measurements[light_source])
        measurements[:, 1] -= baseline_lux
        ind = measurements[:, 1] < 0
        measurements[:, 1][ind] = 0
        devices[device].light_char_measurements[light_source] = measurements
    print(devices[device])
